// /api/interactive-lab.js
// Vercel serverless function for the Cloud & Capital Interactive Lab
// Replaces the older interactive-lab endpoint with the new Decision Frame schema.
...
export default async function handler(req, res) {
  ...
  const { decision, perspective } = req.body || {};

  if (!decision || !perspective) {
    return res.status(400).json({ error: 'Missing required fields: decision, perspective' });
  }

  const perspContext = PERSPECTIVE_CONTEXT[perspective];
  if (!perspContext) {
    return res.status(400).json({ error: 'Invalid perspective. Must be: finops, finance, or engineering' });
  }
